...
from human_shape.utils import StringList, Tensor
...
def build_prior(prior_type, rho=100, reduction="mean", size_average=True, **kwargs):
...
class ThresholdPrior(nn.Module):
    def __init__(self, reduction="mean", margin=1, norm="l2", epsilon=1e-7, **kwargs):
...
class PenaltyPrior(nn.Module):
    ...
    def forward(self, module_input, *args, **kwargs):
        ...
        if self.use_vector:
            ...
            if self.norm == "l1":
                param_norm = module_input.abs().view(batch_size, -1).sum(dim=-1)
                margin = self.margin
            elif self.norm == "l2":
                param_norm = module_input.pow(2).view(batch_size, -1).sum(dim=-1)
                margin = self.margin**2

            thresholded_vals = F.relu(param_norm - margin)
            non_zeros = thresholded_vals.gt(0).to(torch.float32).sum() + self.epsilon
            ...
            if self.norm == "l1":
                return (
                    upper_margin.abs().sum() / upper_non_zeros
                    + lower_margin.abs().sum() / lower_non_zeros
                )
            elif self.norm == "l2":
                return (
                    upper_margin.pow(2).sum() / upper_non_zeros
...
class BarrierPrior(nn.Module):
    ...
    def forward(self, module_input, *args, **kwargs):
        if self.barrier == "log":
            loss = (
                -torch.log(self.margin)
                - torch.log(-(module_input - self.margin) + self.epsilon).mean()
            )
            if self.symmetric:
                loss += (
                    -torch.log(self.margin)
...
class GMMPrior(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if dtype == torch.float32:
            ...
            raise ValueError("Unknown float type {}.format(exiting)!".format(dtype))
        ...
        cov_dets = [np.log(np.linalg.det(covs[idx])) for idx in range(covs.shape[0])]
    ...
    def max_log_likelihood(self, pose, *args):
        ...
        curr_loglikelihood = -0.5 * (
            diff_prec_quadratic + self.cov_dets + self.random_var_dim * self.pi_term
        )
    ...
    def forward(self, pose, *args):
        if len(pose.shape) == 4:
            ...
            pose = (
                kornia.rotation_matrix_to_angle_axis(F.pad(pose, (0, 1)).view(-1, 3, 4))
                .contiguous()
                .view(batch_size, -1)
            )
...
class NormalPrior(nn.Module):
    def __init__(self, stats_path: str, reduction="mean", **kwargs) -> None:
        ...
        assert osp.exists(stats_path), f"Statistics path does not exist: {stats_path}"
    ...
    def forward(self, module_input, *args, **kwargs) -> float:
        if len(module_input) < 1:
            return torch.tensor(0, dtype=module_input.dtype, device=module_input.device)
...
class GenderShapePrior(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if prior_type == "normal":
            self.female_prior = NormalPrior(female_stats_path, reduction="none")
    ...
    def extra_repr(self) -> str:
        msg = [f"Reduction: {self.reduction_str}"]
        return "\n".join(msg)
    ...
    def forward(
        ...
        **kwargs,
        ...
        loss = (
            female_prior_loss.sum() + male_prior_loss.sum() + neutral_prior_loss.sum()
        )
