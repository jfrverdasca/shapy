...
def build_loss(
    type="l2", rho=100, reduction="mean", size_average=True, ignore_index=-100, **kwargs
) -> nn.Module:
    ...
    if type == "gmof":
        ...
        return WeightedL1Loss(reduction=reduction, size_average=size_average, **kwargs)
        ...
        return nn.CrossEntropyLoss(reduction=reduction, ignore_index=ignore_index)
    elif type == "l1":
        return nn.L1Loss()
    elif type == "rotation":
...
def build_adv_loss(discriminator, disc_cfg):
    ...
    if adv_loss_type == "lsgan":
        ...
        return WassersteinGANGP(discriminator, weight=weight, **disc_cfg.wgan_gp)
    else:
        raise ValueError("Unknown adversarial loss: {}".format(adv_loss_type))


class SmoothL1LossModule(nn.Module):
    def __init__(self, size_average=True, beta=1.0 / 9):
        super(SmoothL1LossModule, self).__init__()
        self.size_average = size_average
        self.beta = beta

    def extra_repr(self):
        return "beta={}, size_average={}".format(self.beta, self.size_average)
...
class KeypointLoss(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.robustifier = build_robustifier(robustifier_type=robustifier, **kwargs)
        self.epsilon = epsilon

        DIVISION_CHOICES = ["batch", "visible"]
        assert (
            division in DIVISION_CHOICES
        ), f"division must be one of {DIVISION_CHOICES}, but got {division}"
        self.division = division
        NORMALIZE_CHOICES = ["none", "mean,std"]
        assert (
            normalize in NORMALIZE_CHOICES
    ...
    def forward(self, input, target, weights=None, epsilon=1e-9):
        ...
        if self.division == "batch":
            ...
            return torch.sum(weighted_diff) / (torch.sum(visibility) * 2.0 + epsilon)
...
class GMofLoss(nn.Module):
    ...
    def extra_repr(self):
        return "rho={}, reduction={}".format(self.rho, self.reduction_str)
...
class LSGANLoss(nn.Module):
    ...
    def forward(self, fake, real=None, update_gen=True):
        if update_gen:
            ...
            fake_scores, real_scores = torch.split(all_scores, fake.shape[0], dim=0)

            loss_real = torch.mean((real_scores - 1).pow(2).sum(dim=1))
            loss_fake = torch.mean(fake_scores.pow(2).sum(dim=1))

            return (loss_real + loss_fake) * self.weight, {"real_scores": real_scores}
...
class WassersteinGANGP(nn.Module):
    ...
    def forward(self, fake, real=None, update_gen=True):
        if update_gen:
            ...
            return loss + gp, {"wasserstein_distance": loss_real - loss_fake, "gp": gp}
    ...
    def compute_gp(self, fake, real):
        ...
        num_dims = real.dim() - 1
        tau = torch.zeros([batch_size] + [1] * num_dims, requires_grad=True).to(
        ...
        penalty = ((gradients.norm(2, dim=1) - self.gamma) ** 2).mean(
            dim=0
        ) / self.gamma**2
        return penalty
...
class VertexEdgeLoss(nn.Module):
    ...
    def compute_edges(self, points, connections):
        start = torch.index_select(points, 1, connections[:, 0])
...
class KeypointEdgeLoss(nn.Module):
    ...
    def compute_edges(self, keypoints, connections):
        start = torch.index_select(keypoints, 1, connections[:, 0])
    ...
    def forward(self, gt_keypoints, model_keypoints, connections=None, weights=None):
        ...
        if not torch.is_tensor(connections):
            connections = torch.tensor(
                connections, dtype=torch.long, device=device
            ).reshape(-1, 2)
        ...
        if self.robustifier is not None:
            ...
            if self.norm_type == "l2":
                return (
                    raw_edge_diff.pow(2).sum(dim=-1) * edge_weight
                ).sum() / gt_keypoints.shape[0]
            elif self.norm_type == "l1":
                return (
                    raw_edge_diff.abs().sum(dim=-1) * edge_weight
                ).sum() / gt_keypoints.shape[0]
            else:
                raise NotImplementedError(
