...
class EqualSampler(dutils.Sampler):
    ...
    def __init__(self, datasets, batch_size=1, ratio_2d=0.5, shuffle=False):
        ...
        for dset in datasets:
            ...
            self.elements_per_index[dset.name()] = dset.get_elements_per_index()

            start += len(dset)

        if ratio_2d < 1.0 and sum(self.only_2d.values()) == len(self.only_2d):
            raise ValueError(f"Invalid 2D ratio value: {ratio_2d} with only 2D data")
    ...
    def __repr__(self):
        ...
        for dset_name in self.dset_sizes:
            msg += "\t{}: {}, only 2D is {}\n".format(
                dset_name, self.dset_sizes[dset_name], self.only_2d[dset_name]
            )
    ...
    def _prepare_batches(self):
        ...
        for dset_name, dset_size in self.dset_sizes.items():
            if self.shuffle:
                dset_idxs[dset_name] = cycle(iter(torch.randperm(dset_size).tolist()))
            else:
        ...
        for bidx in range(num_batches):
            ...
            while num_samples < self.batch_size:
                for dset_name in dset_idxs:
                    ...
                    try:
                        curr_idxs.append(
                            next(dset_idxs[dset_name]) + self.offsets[dset_name]
                        )
                        num_samples += self.elements_per_index[dset_name]
                        # If the dataset has only 2D annotations increase the
                        # count
                        num_2d_only += (
                            self.elements_per_index[dset_name] * self.only_2d[dset_name]
                        )
                        idxs_add[dset_name] += self.elements_per_index[dset_name]
                    finally:
                        pass
...
class ShapeSampler(dutils.Sampler):
    ...
    def __init__(
        ...
        for dset in datasets:
            ...
            self.elements_per_index[dset.name()] = dset.get_elements_per_index()
            self.only_2d[dset.name()] = dset.only_2d()
            self.importance_weights[dset.name()] = getattr(dset, importance_key)
    ...
    def _prepare_batches(self):
        ...
        for gender in self.gender_labels:
            ...
            for dset_name, gender_array in self.genders.items():
                gender_idxs[gender][dset_name] = np.where(gender_array == gender)[0]
                importance_probs = self.importance_probs[dset_name][
