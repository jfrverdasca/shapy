...
def build_transforms(
    ...
    if is_train and enable_augment:
        ...
        downsample_cat_factors = transf_cfg.get("downsample_cat_factors", (1.0,))
        ...
        motion_blur_kernel_size_min = transf_cfg.get("motion_blur_kernel_size_min", 3)
        motion_blur_kernel_size_max = transf_cfg.get("motion_blur_kernel_size_max", 7)
    else:
        flip_prob = 0.0
        max_size = -1
        #  downsample_prob = 0.0
    ...
    normalize_transform = T.Normalize(transf_cfg.get("mean"), transf_cfg.get("std"))
    ...
    transform = T.Compose(
        [
            ...
            T.RandomRotation(is_train=is_train, rotation_factor=rotation_factor),
            T.ExtremeBodyCrop(
                prob=extreme_crop_prob, torso_upper_body_prob=torso_upper_body_prob
            ),
            crop,
            pixel_noise,
            downsample,
