...
class ExtremeBodyCrop(object):
    def __init__(self, prob=0.0, torso_upper_body_prob=0.5):
        ...
        logger.info(f"ExtremeBodyCrop: {self.prob}, {self.torso_upper_body_prob}")
...
class MotionBlur(object):
    ...
    def __str__(self):
        ...
        return f"MotionBlur(prob={self.prob}, kernel_size={ksize_str})"
    ...
    def __call__(self, image, target, **kwargs):
        ...
        kernel_size = np.random.randint(self.kernel_size_min, self.kernel_size_max)
        if kernel_size % 2 == 0:
            kernel_size += 1
        ...
        blurred_img = cv2.filter2D(image, -1, kernel, borderType=cv2.BORDER_CONSTANT)
        ...
        vector = np.array(
            [(kernel * xshift).sum(), (kernel * yshift).sum()], dtype=np.float32
        )
...
class BBoxCenterJitter(object):
    def __init__(self, factor=0.0, dist="uniform"):
...
class SimulateLowRes(object):
    ...
    def __str__(self) -> str:
        if self.dist == "uniform":
            dist_str = f"{self.dist.title()}: [{self.factor_min}, {self.factor_max}]"
        else:
            dist_str = f"{self.dist.title()}: [{self.cat_factors}]"
        return f"SimulateLowResolution({dist_str})"

    def _sample_low_res(self, image: Union[np.ndarray, pil_img.Image]) -> np.ndarray:
        """ """
        if self.dist == "uniform":
            ...
            if not downsample:
                return image
            factor = (
                np.random.rand() * (self.factor_max - self.factor_min) + self.factor_min
            )
        ...
        resized_image = cv2.resize(downsampled_image, (W, H), cv2.INTER_LINEAR_EXACT)
        ...
        **kwargs,
        ...
        if self.noise_scale > 0:
            ...
            pn = np.random.uniform(
                1 - self.noise_scale, 1 + self.noise_scale, 3
            ).astype(np.float32)
            ...
            if image is not None:
                output_image = np.clip(image * pn[np.newaxis, np.newaxis], 0, 1.0)
    ...
    def __call__(self, image, target, **kwargs):
        ...
        if self.is_train:
            rot = min(
                ...
                max(
                    -2 * self.rotation_factor, np.random.randn() * self.rotation_factor
                ),
            )
        ...
        if target.has_field("intrinsics"):
            ...
            intrinsics[:2, 2] = np.dot(M[:2, :2], cam_center) + M[:2, 2]
        ...
        if target.has_field("translation") and target.has_field("pelvis"):
            ...
            if is_tensor:
                new_translation = torch.tensor(new_translation, dtype=torch.float32)
...
class Crop(object):
    ...
    def __call__(self, image, target, **kwargs):
        ...
        cropped_image = crop(np_image, center, scale, [self.crop_size, self.crop_size])
        cropped_target = target.crop(center, scale, crop_size=self.crop_size)

        transf = get_transform(center, scale, [self.crop_size, self.crop_size])

        if target.has_field("mask"):
            mask = target.get_field("mask")
            cropped_mask = crop(mask, center, scale, [self.crop_size, self.crop_size])
        ...
        if target.has_field("intrinsics"):
            ...
            cam_center = intrinsics[:2, 2]
            intrinsics[:2, 2] = np.dot(transf[:2, :2], cam_center) + transf[:2, 2]
            cropped_target.add_field("intrinsics", intrinsics)

        return (
            np_image if self.return_full_imgs else None,
            cropped_image,
            cropped_target,
        )
...
class Resize(object):
    ...
    def __call__(self, image, target, **kwargs):
        ...
        output_image = cv2.resize(
            image, (int(ratio * W), int(ratio * H)), interpolation=cv2.INTER_AREA
        )
        ...
        if target.has_field("mask"):
            ...
            resized_mask = cv2.resize(
                mask, (int(ratio * W), int(ratio * H)), interpolation=cv2.INTER_AREA
            )
        ...
        resized_target.add_field("orig_center", target.get_field("orig_center") * ratio)
...
class Normalize(object):
    ...
    def __call__(self, image, cropped_image, target, **kwargs):
        ...
        output_cropped_image = F.normalize(cropped_image, mean=self.mean, std=self.std)
