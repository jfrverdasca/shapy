...
from human_shape.utils import Array, StringList, binarize, nand
...
def threshold_and_keep_parts(
    ...
    if binarization:
        body_conf = binarize(body_conf, body_thresh, keypoints.dtype)
        left_hand_conf = binarize(left_hand_conf, hand_thresh, keypoints.dtype)
        right_hand_conf = binarize(right_hand_conf, hand_thresh, keypoints.dtype)
        face_conf = binarize(face_conf, face_thresh, keypoints.dtype)
...
def read_keypoints(keypoint_fn, use_hands=True, use_face=True, use_face_contour=True):
    ...
    for idx, person_data in enumerate(data["people"]):
        ...
        left_hand_keyps = np.array(left_hand_keyps, dtype=np.float32).reshape([-1, 3])

        right_hand_keyps = person_data.get("hand_right_keypoints_2d", [])
        if len(right_hand_keyps) < 1:
            right_hand_keyps = [0] * (21 * 3)
        right_hand_keyps = np.array(right_hand_keyps, dtype=np.float32).reshape([-1, 3])

        face_keypoints = person_data.get("face_keypoints_2d", [])
        if len(face_keypoints) < 1:
            face_keypoints = [0] * (70 * 3)

        face_keypoints = np.array(face_keypoints, dtype=np.float32).reshape([-1, 3])

        face_keypoints = face_keypoints[:-2]

        all_keypoints.append(
            np.concatenate(
                [body_keypoints, left_hand_keyps, right_hand_keyps, face_keypoints],
                axis=0,
            )
        )
...
def map_keypoints(
    ...
    assert (
        source_dataset in names_dict
    ), f"Source dataset not in names dictionary: {source_dataset}"
    assert (
        target_dataset in names_dict
...
def kp_connections(
    ...
    for connection in kp_lines:
        if connection[0] in keypoints and connection[1] in keypoints:
            if part is None:
                connections.append(
                    [keypoints.index(connection[0]), keypoints.index(connection[1])]
                )
            else:
...
def get_part_idxs(
    keypoint_names: List[str], keypoint_parts: Dict[str, str]
) -> Dict[str, Array]:
    ...
    torso_idxs = np.asarray(
        [idx for idx, val in enumerate(keypoint_parts.values()) if "torso" in val]
    )
    upper_body_idxs = np.asarray(
        [idx for idx, val in enumerate(keypoint_parts.values()) if "upper" in val]
    )
    return {
