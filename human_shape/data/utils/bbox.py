...
from human_shape.utils import Tensor
...
__all__ = [
    ...
    "bbox_to_wh",
    "bbox_iou",
    "bbox_xyxy_to_bbox_xywh",
    "bbox_xywh_to_bbox_xyxy",
]


def points_to_bbox(
    points: Tensor, bbox_scale_factor: float = 1.0
) -> Tuple[Tensor, Tensor]:
...
def keyps_to_bbox(
    keypoints, conf, img_size=None, clip_to_img=False, min_valid_keypoints=6, scale=1.0
):
...
def bbox_to_center_scale(bbox, dset_scale_factor=1.0, ref_bbox_size=200):
    ...
    bbox = bbox.reshape(-1)
    bbox_size = dset_scale_factor * max(bbox[2] - bbox[0], bbox[3] - bbox[1])
...
def bbox_xyxy_to_bbox_xywh(bbox):
    if bbox is None:
        return (0.0, 0.0, 0.0, 0.0)
    bbox = np.asarray(bbox)
    xmin, ymin, xmax, ymax = np.split(bbox.reshape(-1, 4), 4, axis=1)
    return np.array([xmin, ymin, xmax - xmin, ymax - ymin], dtype=bbox.dtype)


def bbox_xywh_to_bbox_xyxy(bbox):
    if bbox is None:
        return (0.0, 0.0, 0.0, 0.0)
    bbox = np.asarray(bbox)
    xmin, ymin, width, height = np.split(bbox.reshape(-1, 4), 4, axis=1)
    out_bbox = np.array([xmin, ymin, xmin + width, ymin + height], dtype=bbox.dtype)
    return out_bbox.reshape(-1, 4)
...
def bbox_iou(bbox1, bbox2, epsilon=1e-9):
    ...
    if torch.is_tensor(bbox1):
        ...
        wh = np.clip(right_bottom - left_top, 0, float("inf"))
        isect = wh[0] * wh[1]
