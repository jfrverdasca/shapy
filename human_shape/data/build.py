...
from .datasets.image import ImageDataset, ImageWithBoxesDataset
...
def build_shape_dataset(
    name: str, dataset_cfg: DictConfig, transforms, **kwargs
) -> None:
    ...
    vertex_flip_correspondences = osp.expandvars(
        dataset_cfg.get("vertex_flip_correspondences", "")
    )
    dset_obj = obj(
...
class MemoryPinning(object):
    ...
    def pin_memory(
        self,
    ) -> Tuple[Union[ImageList, ImageListPacked, TensorList], Tensor, StructureList]:
...
def collate_batch(
    batch, use_shared_memory=False, return_full_imgs=False, pin_memory=False
):
    if return_full_imgs:
...
def build_sampler(
    ...
    if part_key == "pose":
        ...
        if use_equal_sampling and is_train:
            ...
            out_dsets_lst = [
                dutils.ConcatDataset(datasets) if len(datasets) > 1 else datasets[0]
            ]
        else:
...
def build_data_loader(
    ...
    if batch_sampler is None:
        sampler = build_data_sampler(
            dataset, is_train=is_train, shuffle=shuffle, is_distributed=is_distributed
        )
...
def build_all_data_loaders(
    ...
    **kwargs,
    ...
    batch_sizes = {
        "pose": int(round(batch_size * pose_shape_ratio)),
        "shape": int(round(batch_size * (1 - pose_shape_ratio))),
    }

    data_loaders = {"pose": [], "shape": []}
    build_dset_foo = {"pose": build_pose_dataset, "shape": build_shape_dataset}
    ...
    for part_key in ["pose", "shape"]:
        ...
        logger.info("Body transformations: \n{}", "\n".join(list(map(str, transforms))))
        ...
        num_workers = dsets_cfg.get("num_workers", DEFAULT_NUM_WORKERS).get(split, 0)
...
def preprocess_images(
    ...
    dataset = ImageDataset(image_folder, transforms=transform)
    ...
    bboxes, img_paths = calculate_bounding_boxes(
        device, min_score, rcnn_dataloader, rcnn_model
    )

    dataset_cfg = exp_cfg.get("datasets", {}).get("shape")
    transfs_cfg = dataset_cfg.get("transforms", {})
    transforms = build_transforms(
        transfs_cfg,
        is_train=False,
        enable_augment=True,
    ...
    expose_dset = ImageWithBoxesDataset(
    ...
    expose_dataloader = dutils.DataLoader(
        expose_dset,
        batch_size=1,
        num_workers=num_workers,
