...
from human_shape.utils import binarize, read_img

from ..structures import Joints, Keypoints2D, Vertices
from ..utils import (
    KEYPOINT_NAMES_DICT,
    KEYPOINT_PARTS,
...
class ThreeDPW(dutils.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        if self.split == "train":
            ...
            npz_fn = osp.join(self.data_folder, param_folder, "3dpw_validation.npz")
        elif self.split == "test":
            seq_split_path = osp.join(seq_path, "test")
            npz_fn = osp.join(self.data_folder, param_folder, "3dpw_test.npz")

        self.vertex_folder = osp.join(self.data_folder, vertex_folder, self.split)
        ...
        if "keypoints2d" in data_dict:
            self.keypoints2d = np.asarray(data_dict["keypoints2d"]).astype(np.float32)[
                idxs
            ]
        ...
        self.joints3d = np.asarray(data_dict["joints3d"]).astype(np.float32)[idxs]
        self.num_items = len(self.img_paths)
        self.pids = np.asarray(data_dict["pid"], dtype=np.int32)
        self.center = np.asarray(data_dict["center"], dtype=np.float32)[idxs]
        self.scale = np.asarray(data_dict["scale"], dtype=np.float32)[idxs]
        self.bbox_size = np.asarray(data_dict["bbox_size"], dtype=np.float32)[idxs]
    ...
    def __getitem__(self, index):
        ...
        if self.binarization:
            body_conf = binarize(body_conf, self.body_thresh, keypoints2d.dtype)
        ...
        if self.return_vertices:
            vertex_fname = osp.join(
                self.vertex_folder, f"{index + self.idxs[0]:06d}.npy"
            )
        ...
        if self.transforms is not None:
            img, cropped_image, target = self.transforms(img, target, force_flip=False)
