...
from human_shape.utils import binarize, nand, read_img
...
class OpenPose(dutils.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        for img_fname in os.listdir(self.img_folder):
            ...
            keyp_path = osp.join(self.keyp_folder, "{}_keypoints.json".format(fname))
            if not osp.exists(keyp_path):
                keyp_path = osp.join(self.keyp_folder, "{}.json".format(fname))
                if not osp.exists(keyp_path):
                    continue
    ...
    def __getitem__(self, index):
        ...
        if self.binarization:
            ...
            face_conf = binarize(face_conf, self.face_thresh, keypoints2d.dtype)
...
class OpenPoseTracks(dutils.Dataset):
    def __init__(
        self,
        data_folder="data/openpose_tracks",
        img_folder="images",
        keyp_folder="keypoints",
        split="train",
        head_only=False,
