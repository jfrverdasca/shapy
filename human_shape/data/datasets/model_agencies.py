...
from human_shape.utils import Tensor, batch_rot2aa, read_img
    ...
    for agency in tqdm(keypoint_data, desc="Modeling agencies", leave=False):
        ...
        for model_name in tqdm(keypoint_data[agency], leave=False):
            ...
            agency_output["identity"] += [
                identities[(agency, model_name)]
            ] * num_instances

            #  logger.info(f'{agency}: {model_name}')
            if betas_dict is not None:
                agency_output["betas"] += [
                    betas_dict[agency][model_name]
                ] * num_instances
            if weights_dict is not None:
                agency_output["weight"] += [
                    weights_dict[agency][model_name]
                ] * num_instances
...
def parse_measurement(measurement):
    if type(measurement) is float:
        ...
        return float(measurement.replace(",", ".")) / 100 if measurement else -1
...
class ModelAgency(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.return_params:
            logger.info(f"Loading estimated parameters from: {self.param_folder}")

            # If we are loading parameters, then make sure that we load the
            # proper transformation to flip vertices
            vertex_flip_correspondences = osp.expandvars(vertex_flip_correspondences)
            err_msg = (
        ...
        assert osp.exists(annot_path), f"Could not find annotation path: {annot_path}"
        ...
        if split == "train" or split == "test":
            attr_mask = (
                output["has_attributes"]
                if only_data_with_attributes
                else np.ones(len(output["agency"]))
            )
        else:
    ...
    def __getitem__(self, index):
        ...
        keypoints2d = np.concatenate(
            [body_keypoints, left_hand_keypoints, right_hand_keypoints, face_keypoints],
            axis=0,
        )
        ...
        if self.return_params and osp.exists(param_path):
            ...
            vertices_field = Vertices(
                vertices.reshape(-1, 3), bc=self.bc, closest_faces=self.closest_faces
            )
