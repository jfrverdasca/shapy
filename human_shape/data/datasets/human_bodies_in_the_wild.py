...
class HumanBodyInTheWild(dutils.Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Load SMPL model
        body_model = smplx.create(model_path=body_model_folder, model_type=model_type)
        ...
        for subject_folder in tqdm.tqdm(subject_folders):
            ...
            if self.split in ["val", "test"]:
                curr_v_shaped = (
                    torch.from_numpy(gt_v_shaped[subject_id]).unsqueeze(0).float()
                )
            ...
            for img_type in tqdm.tqdm(os.listdir(curr_subj_path), desc="Type"):
                ...
                keyp_path = osp.join(self.keyp_folder, subject_folder, img_type)

                for img_fname in tqdm.tqdm(os.listdir(img_type_path), desc="Images"):
                    if img_fname.startswith("."):
                        continue

                    curr_img_path = osp.join(img_type_path, img_fname)
                    assert osp.exists(curr_img_path), f"{curr_img_path} not found"

                    fname, _ = osp.splitext(img_fname)
                    curr_keyp_path = osp.join(keyp_path, f"{fname}.json")

                    if not osp.exists(curr_keyp_path):
                        correct_fname = (
                            fname.replace("(", "").replace(")", "").replace(" ", "_")
                        )
                        curr_keyp_path = osp.join(
                            keyp_path, f"{correct_fname}" + ".json"
                        )
        ...
        logger.warning(
            f"Skipped {skip_multi_counter} images, because of != 1 OpenPose detections."
        )
    ...
    def __getitem__(self, index):
        ...
        center, scale, bbox_size = bbox_to_center_scale(bbox, dset_scale_factor=1.2)
        ...
        if self.transforms is not None:
            ...
            img, cropped_image, target = self.transforms(
                img, target, dset_scale_factor=1.2
            )
