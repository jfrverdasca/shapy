...
from human_shape.utils import Tensor, Timer


class ImageList(object):
    def __init__(self, images: torch.Tensor, img_sizes: List[torch.Size], padding=None):
        self.images = images
        self.img_sizes = img_sizes
        self.sizes_tensor = torch.stack(
            [torch.tensor(s) if not torch.is_tensor(s) else s for s in img_sizes]
        ).to(dtype=self.images.dtype)
        if padding is not None:
            self.padding_tensor = torch.stack(
                [torch.tensor(s) if not torch.is_tensor(s) else s for s in padding]
            ).to(dtype=self.images.dtype)
...
class ImageListPacked(object):
    ...
    def as_image_list(self) -> List[Tensor]:
        ...
        for ii in range(len(self.img_sizes)):
            ...
            out_img = torch.zeros(out_shape, device=self.device, dtype=self.dtype)
            out_img[:c, :h, :w] = img
...
def to_image_list_concat(images: List[Tensor]) -> ImageList:
    if images is None:
        return images
    ...
    batched = torch.zeros(batched_shape, device=images[0].device, dtype=images[0].dtype)
...
def to_image_list(
    images: List[Tensor], use_packed=False
