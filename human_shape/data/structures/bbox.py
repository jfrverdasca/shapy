...
class BoundingBox(AbstractStructure):
    ...
    def rotate(self, rot=0, *args, **kwargs):
        ...
        bbox_target = type(self)(new_bbox, size=(nH, nW, 3), transform=self.transform)
        for k, v in self.extra_fields.items():
            if isinstance(v, AbstractStructure):
    ...
    def crop(self, center, scale, rot=0, crop_size=224, *args, **kwargs):
        if self.transform:
            ...
            transf = get_transform(center, scale, (crop_size, crop_size), rot=rot)
        ...
        bbox_target = type(self)(
            new_bbox, size=(crop_size, crop_size), transform=self.transform
        )
        for k, v in self.extra_fields.items():
            if isinstance(v, AbstractStructure):
                v = v.crop(
    ...
    def shift(self, vector, *args, **kwargs):
        ...
        if torch.is_tensor(self.bbox):
            new_bbox = torch.tensor(
                [xmin + vector[0], ymin + vector[1], xmax + vector[0], ymax + vector[1]]
            )
        else:
            new_bbox = np.array(
                [xmin + vector[0], ymin + vector[1], xmax + vector[0], ymax + vector[1]]
        ...
        bbox_target = type(self)(
            new_bbox, size=self.size, flip_axis=self.flip_axis, transform=self.transform
        )
        for k, v in self.extra_fields.items():
            if isinstance(v, AbstractStructure):
    ...
    def resize(self, size, *args, **kwargs):
        ...
        new_bbox = np.array([xmin, ymin, xmax, ymax])
        bbox_target = type(self)(
            new_bbox, size=size, flip_axis=self.flip_axis, transform=self.transform
        )
        for k, v in self.extra_fields.items():
            if isinstance(v, AbstractStructure):
    ...
    def transpose(self, method):
        ...
        if torch.is_tensor(self.bbox):
            ...
            flipped_bbox = np.array(
                [new_xmin, new_ymin, new_xmax, new_ymax], dtype=self.bbox.dtype
            )

        bbox_target = type(self)(flipped_bbox, self.size, transform=self.transform)
    ...
    def to(self, *args, **kwargs):
        ...
        bbox_target = type(self)(
            bbox_tensor.to(*args, **kwargs), self.size, transform=self.transform
        )
        for k, v in self.extra_fields.items():
            if hasattr(v, "to"):
                v = v.to(*args, **kwargs)
