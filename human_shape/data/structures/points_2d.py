...
from human_shape.utils import Array, IntTuple, Tensor, get_transform
...
class Points2D(AbstractStructure):
    ...
    def transpose(self, method):
        ...
        if self.bc is not None:
            ...
            flipped_points = (self.bc[:, :, np.newaxis] * closest_tri_points).sum(
                axis=1
            )
    ...
    def crop(self, center, scale, crop_size=224, *args, **kwargs):
        ...
        points = (np.dot(points, transf[:2, :2].T) + transf[:2, 2] + 1).astype(
            points.dtype
        )
    ...
    def rotate(self, rot=0, *args, **kwargs):
        ...
        M[0, 2] += (nW / 2) - cX
        M[1, 2] += (nH / 2) - cY
        points = (np.dot(points, M[:2, :2].T) + M[:2, 2] + 1).astype(points.dtype)
    ...
    def to(self, *args, **kwargs):
        points = type(self)(
            self.points.to(*args, **kwargs), size=self.size, flip_axis=self.flip_axis
        )
        for k, v in self.extra_fields.items():
            if hasattr(v, "to"):
                v = v.to(*args, **kwargs)
