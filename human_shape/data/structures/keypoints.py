...
from human_shape.utils import Array, IntTuple, StringList, Tensor
...
class Keypoints2D(AbstractStructure):
    def __init__(
        ...
        **kwargs,
        ...
        if self._names is None:
            error_msg = (
                f"{self.source} not in names dict."
        ...
        if _connections is None:
            error_msg = (
                f"{self.source} not in connections dict."
                f" Please make sure that {self.source} is a key"
                f" in the keypoint dictionary!"
            )
            raise ValueError(error_msg)
        ...
        if _parts is None:
            error_msg = (
                f"{self.source} not in parts dict."
                f" Please make sure that {self.source} is a key"
                f" in the keypoint dictionary!"
            )
            raise ValueError(error_msg)
    ...
    def torso_crop(self):
        ...
        bbox = keyps_to_bbox(torso_body_keyps, torso_body_conf, img_size=self.size)
    ...
    def upper_body_crop(self, dset_scale_factor=1.0):
        ...
        bbox = keyps_to_bbox(upper_keyps, upper_conf, img_size=self.size)
        center, scale, bbox_size = bbox_to_center_scale(
            bbox,
            dset_scale_factor=dset_scale_factor,
        )
    ...
    def to_dset(
        ...
        if torch.is_tensor(self.keypoints):
            ...
            output_conf = torch.zeros([target_dim], dtype=dtype, device=device)
            ...
            output_keypoints = np.zeros([target_dim, self.dim], dtype=dtype)
        ...
        if output == "tensor":
            if not torch.is_tensor(output_keypoints):
                output_keypoints = torch.from_numpy(output_keypoints).to(
                    dtype=torch.float32
                )
                output_conf = torch.from_numpy(output_conf).to(dtype=torch.float32)
            ...
            if torch.is_tensor(output_keypoints):
                ...
                keyps = np.concatenate([output_keypoints, output_conf], axis=-1)
            return Keypoints2D(
                keyps,
                size=self.size,
    ...
    def crop(self, center, scale, crop_size=224, *args, **kwargs):
        if not self.apply_crop:
            for k, v in self.extra_fields.items():
                if isinstance(v, AbstractStructure):
                    v = v.crop(
                        center=center, scale=scale, crop_size=crop_size, *args, **kwargs
                    )
                self.add_field(k, v)
            return self
        kp = self.keypoints.copy()
    ...
    def transpose(self, method):
        ...
        TO_REMOVE = 1
        if torch.is_tensor(self.keypoints):
            flipped_data = torch.cat(
                [self.keypoints, self.conf.unsqueeze(dim=-1)], dim=-1
            )
            ...
            #  TO_REMOVE = 1
            flipped_data[..., :, self.flip_axis] = (
                width - flipped_data[..., :, self.flip_axis] - TO_REMOVE
            )
        else:
    ...
    def to(self, *args, **kwargs):
        keyp_tensor = torch.cat([self.keypoints, self.conf.unsqueeze(dim=-1)], dim=-1)
        keypoints = type(self)(
            keyp_tensor.to(*args, **kwargs),
            self.size,
            source=self.source,
            flip_indices=self.flip_indices,
            apply_crop=self.apply_crop,
