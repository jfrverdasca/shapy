...
class HandPose(AbstractStructure):
        ...
        if to_rot:
            if self.left_hand_pose is not None:
                self.left_hand_pose = batch_rodrigues(
                    self.left_hand_pose.view(-1, 3)
                ).view(-1, 3, 3)
            if self.right_hand_pose is not None:
                self.right_hand_pose = batch_rodrigues(
                    self.right_hand_pose.view(-1, 3)
                ).view(-1, 3, 3)
    ...
    def transpose(self, method):
        ...
        if self.right_hand_pose is not None:
            left_hand_pose = (self.right_hand_pose.reshape(15, 3) * dim_flip).reshape(
                45
            )
        if self.left_hand_pose is not None:
            right_hand_pose = (self.left_hand_pose.reshape(15, 3) * dim_flip).reshape(
                45
            )
