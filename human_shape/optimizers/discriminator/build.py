...
class PartDiscriminators(nn.Module):
    def __init__(self, num_input, num_parts, bias=True, use_spectral_norm=True):
        ...
        if bias:
            ...
            self.register_parameter("bias", nn.Parameter(biases, requires_grad=True))

    def forward(self, part_input):
        return torch.einsum(
            "bcj,jc->bj", [part_input, self.weights]
        ) + self.bias.unsqueeze(dim=0)


class HMRDiscriminator(nn.Module):
    def __init__(
        self, num_joints=21, num_channels=32, nzfeat=1024, use_spectral_norm=True
    ):
    ...
    def forward(self, pose):
        ...
        part_probs = (
            self.part_disc(pose.permute(0, 2, 1))
            .reshape(batch_size, num_joints)
            .reshape(batch_size, num_joints)
        )
