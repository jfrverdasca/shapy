    ...
    def load_checkpoint(self):
        ...
        if not osp.exists(save_fn):
            ...
            if len(self.pretrained) > 1:
                ...
                save_fn = osp.join(self.pretrained, "checkpoints", "latest_checkpoint")
        ...
        if len(unexpected):
            logger.warning(f"The following keys were not expected: {unexpected}")

        if self.optimizer is not None and "optimizer" in ckpt_data:
            if not load_pretrained:
                logger.warning("Loading optimizer data from: {}".format(self.save_dir))
                self.optimizer.load_state_dict(ckpt_data["optimizer"])

        if self.scheduler is not None and "scheduler" in ckpt_data:
            if not load_pretrained:
                logger.warning("Loading scheduler data from: {}".format(self.save_dir))
                self.scheduler.load_state_dict(ckpt_data["scheduler"])
        if self.adv_optimizer is not None and "adv_optimizer" in ckpt_data:
            if not load_pretrained:
                logger.warning(
                    "Loading discriminator optim data from: {}".format(self.save_dir)
                )
                self.adv_optimizer.load_state_dict(ckpt_data["adv_optimizer"])
