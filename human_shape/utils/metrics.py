...
from .typing import Array, IntList, Tensor
...
def point_error(
    input_points: Union[Array, Tensor], target_points: Union[Array, Tensor]
) -> Array:
...
def mpjpe(
    input_joints: Union[Array, Tensor], target_joints: Union[Array, Tensor]
) -> Array:
...
class ProcrustesAlignment(object):
    ...
    def __call__(self, S1: Array, S2: Array) -> Tuple[Array, Array]:
        ...
        # 7. Error:
        S1_hat = scale.reshape(-1, 1, 1) * (R @ S1) + t.reshape(batch_size, -1, 1)
...
def point_fscore(
    pred: torch.Tensor, gt: torch.Tensor, thresh: float
) -> Dict[str, float]:
...
class PointError(object):
    ...
    def set_alignment(
        self, alignment_object: Union[ProcrustesAlignment, RootAlignment, NoAlignment]
    ) -> None:
        self._alignment = alignment_object
...
class v2vhdError(torch.nn.Module):
    ...
    def to_pytorch(self, point_regressor):
        ...
        indices = np.vstack((point_regressor.row, point_regressor.col))
        values = point_regressor.data
        shape = point_regressor.shape
        point_regressor = torch.sparse_coo_tensor(indices, values, shape)
    ...
    def get_translation(self, gt_points, pred_points):
        if self.align:
            ...
            B = len(gt_points)
            t = torch.zeros([B, 3], dtype=gt_points.dtype, device=pred_points.device)
        return t

    def __call__(self, input_points, target_points):

        hd_input_points = self.sparse_batch_mm(self.input_point_regressor, input_points)
