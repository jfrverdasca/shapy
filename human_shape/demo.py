...
from human_shape.data.build import preprocess_images
...
from human_shape.utils import COLORS, Checkpointer, HDRenderer, OverlayRenderer
...
def weak_persp_crop_to_blender(
    targets, camera_scale, camera_transl, crop_size, sensor_width=36, focal_length=5000
):
    ...
    for ii, target in enumerate(targets):
        ...
        transl = [camera_transl[ii, 0].item(), camera_transl[ii, 1].item(), z]
        shift_x = 0.0
        shift_y = 0.0
...
def weak_persp_to_blender(
    targets, camera_scale, camera_transl, H, W, sensor_width=36, focal_length=5000
):
    """Converts weak-perspective camera to a perspective camera"""
    if torch.is_tensor(camera_scale):
        camera_scale = camera_scale.detach().cpu().numpy()
    ...
    for ii, target in enumerate(targets):
        ...
        transl = [camera_transl[ii, 0].item(), camera_transl[ii, 1].item(), z.item()]
...
def undo_img_normalization(image, mean, std, add_alpha=True):
    ...
    out_img = (
        image * std[np.newaxis, :, np.newaxis, np.newaxis]
...
def main(
    ...
    transf_cfg = exp_cfg.get("datasets", {}).get(part_key, {}).get("transforms", {})
    ...
    body_dloader = preprocess_images(image_folder, exp_cfg, batch_size=1, device=device)
    ...
    for bidx, batch in enumerate(tqdm(body_dloader, dynamic_ncols=True)):
        ...
        model_output = model(
            body_imgs, body_targets, full_imgs=full_imgs, device=device
        )
        torch.cpu.synchronize()
        ...
        if render:
            hd_imgs = np.transpose(
                undo_img_normalization(hd_imgs, means, std), [0, 2, 3, 1]
            )
        ...
        if save_vis:
            for key in out_img.keys():
                out_img[key] = np.clip(
                    np.transpose(out_img[key], [0, 2, 3, 1]) * 255, 0, 255
                ).astype(np.uint8)
                if "left" in key:
        ...
        for idx in tqdm(range(len(body_targets)), "Saving ..."):
            ...
            if save_mesh:
                ...
                mesh = trimesh.Trimesh(
                    model_vertices[idx] + hd_params["transl"][idx], faces, process=False
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pause", default=-1, type=float, help="How much to pause the display"
    )
    parser.add_argument(
        "--exp-opts",
        default=[],
