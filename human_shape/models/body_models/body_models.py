...
from human_shape.utils import IntList, StringList, Struct, Tensor, to_np
...
class SMPL(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if osp.exists(v_template_path):
            ...
            v_template_mesh = trimesh.load_mesh(v_template_path, process=False)
        ...
        j14_regressor_path = osp.expandvars(kwargs.get("j14_regressor_path", ""))
        ...
        if self.use_joint_regressor:
            logger.info(f"Loading joint regressor from: {j14_regressor_path}")
            ...
            extra_joint_regressor = torch.from_numpy(j14_regressor).to(
                dtype=torch.float32
            )
            self.register_buffer("extra_joint_regressor", extra_joint_regressor)
    ...
    def connections(self) -> List[Tuple[int, int]]:
        if not hasattr(self, "_connections"):
            connections = kp_connections(self.keypoint_names, KEYPOINT_CONNECTIONS)
    ...
    def build_keypoint_names(self) -> StringList:
        ...
        if hasattr(self, "extra_joint_selector"):
            model_keypoint_names += self.extra_joint_selector.extra_joint_names()
    ...
    def forward(
        ...
        **kwargs,
        ...
        output = defaultdict(lambda: None, joints=output_joints, faces=self.faces)
        if get_skin:
...
class SMPLH(SMPL):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(SMPLH, self).__init__(
            ...
            gender=gender,
            dtype=dtype,
            single_vertex_per_tip=single_vertex_per_tip,
            **kwargs,
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        full_pose = torch.cat(
            [global_rot, body_pose, left_hand_pose, right_hand_pose], dim=1
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_face_contour:
            dyn_lmk_faces_idx, dyn_lmk_b_coords = find_dynamic_lmk_idx_and_bcoords(
                vertices,
                full_pose,
            ...
            lmk_bary_coords = torch.cat(
                [lmk_bary_coords.expand(batch_size, -1, -1), dyn_lmk_b_coords], 1
            )
        ...
        if return_full_pose:
            output["full_pose"] = full_pose
        if return_shaped:
            output["v_shaped"] = self.v_template + blend_shapes(betas, self.shapedirs)
