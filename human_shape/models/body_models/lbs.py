...
def find_dynamic_lmk_idx_and_bcoords(
    ...
    out_dyn_lmk_faces_idx = torch.index_select(dynamic_lmk_faces_idx, 0, y_rot_angle)
    out_dyn_lmk_b_coords = torch.index_select(dynamic_lmk_b_coords, 0, y_rot_angle)
...
def vertices2landmarks(
    ...
    lmk_faces = torch.index_select(faces, 0, lmk_faces_idx.contiguous().view(-1)).view(
        batch_size, -1, 3
    )
...
def batch_rigid_transform(rot_mats, joints, parents):
    ...
    rel_transforms = transforms - F.pad(
        torch.matmul(transforms, joints_homogen), [3, 0, 0, 0, 0, 0, 0, 0], value=0.0
    )
