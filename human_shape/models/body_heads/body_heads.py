...
from human_shape.utils import BlendShapeDescription, StringList, Tensor

from ..body_models import KeypointTensor, build_body_model
from ..common.iterative_regressor import HMRLikeRegressor
from ..common.keypoint_loss import KeypointLosses
from ..common.pose_utils import PoseParameterization, build_pose_parameterization
...
class SMPLRegressor(HMRLikeRegressor):
    ...
    def __init__(
        ...
        super(SMPLRegressor, self).__init__(body_model_cfg, network_cfg, loss_cfg)
    ...
    def _build_keypoint_losses(
        ...
        loss_modules, center_around = super(SMPLRegressor, self)._build_keypoint_losses(
            loss_cfg
        )
        KEYPOINT_PARTS = ["body"]
    ...
    def _build_parameter_losses(self, loss_cfg) -> [SMPLLossModule, RegularizerModule]:
    ...
    def _build_pose_space(
        ...
        param_desc = super(SMPLRegressor, self)._build_pose_space(body_model_cfg)
    ...
    def _build_blendshape_space(
        ...
        if osp.exists(shape_mean_path):
            shape_mean = (
                torch.from_numpy(np.load(shape_mean_path, allow_pickle=True))
                .to(dtype=dtype)
                .reshape(1, -1)[:, :num_betas]
                .reshape(-1)
            )
        else:
...
class SMPLHRegressor(SMPLRegressor):
    ...
    def _build_pose_space(self, body_model_cfg) -> Dict[str, PoseParameterization]:
        param_desc = super(SMPLHRegressor, self)._build_pose_space(body_model_cfg)
    ...
    def _build_parameter_losses(self, loss_cfg) -> [SMPLHLossModule, RegularizerModule]:
    ...
    def _build_keypoint_losses(
        ...
        loss_modules, center_around = super(
            SMPLHRegressor, self
        )._build_keypoint_losses(loss_cfg)
...
class SMPLXRegressor(SMPLHRegressor):
    ...
    def _build_pose_space(self, body_model_cfg) -> Dict[str, PoseParameterization]:
        param_desc = super(SMPLXRegressor, self)._build_pose_space(body_model_cfg)
        if self.predict_face:
            jaw_pose_desc = build_pose_parameterization(
                1,
                mean=self.mean_poses_dict.get("jaw_pose", None),
    ...
    def _build_parameter_losses(self, loss_cfg) -> [SMPLXLossModule, RegularizerModule]:
