...
from human_shape.utils import IntList, StringList, Tensor, TensorList

PARAM_KEYS = ["betas", "expression", "global_rot", "body_pose", "hand_pose", "jaw_pose"]
...
class SMPLLossModule(nn.Module):
    ...
    def single_loss_step(
        ...
        compute_shape_loss = (
            self.shape_weight > 0
            and self.loss_enabled["betas"]
            and "betas" in parameters
            and "betas" in target_params
            and not penalize_only_parts
        )
        if compute_shape_loss:
            shape_common_dim = min(
                parameters["betas"].shape[-1], target_params["betas"].shape[-1]
            )
        ...
        compute_global_rot_loss = (
            self.global_rot_weight > 0
            and self.loss_enabled["betas"]
            and "global_rot" in target_params
            and not penalize_only_parts
        )
        ...
        compute_body_pose_loss = (
            self.body_pose_weight > 0
            and self.loss_enabled["betas"]
            and "body_pose" in target_params
            and not penalize_only_parts
        )
        ...
        if compute_body_pose_loss:
            ...
            losses["body_pose_loss"] = (
                self.body_pose_loss(est_body_pose, target_params["body_pose"])
                * self.body_pose_weight
            )
    ...
    def forward(
        ...
        for idx, target in enumerate(targets):
            ...
            for param_key in PARAM_KEYS:
                ...
                in_target_params[param_key].append(target.get_field(param_key))

        target_params = {}
        for key, val in in_target_params.items():
            if key == "hand_pose":
                target_params["left_hand_pose"] = torch.stack(
                    [t.left_hand_pose for t in val]
                )
                target_params["right_hand_pose"] = torch.stack(
                    [t.right_hand_pose for t in val]
                )
                ...
                target_params[key] = torch.stack([getattr(t, key) for t in val])
...
class SMPLHLossModule(SMPLLossModule):
    def __init__(self, loss_cfg):
        ...
        if self.left_hand_pose_weight > 0:
            ...
            self.loss_activ_step["left_hand_pose"] = loss_cfg.left_hand_pose.enable
            logger.debug(
                "Left hand pose weight, loss: {}, {}",
        ...
        if self.right_hand_pose_weight > 0:
            ...
            self.loss_activ_step["right_hand_pose"] = loss_cfg.right_hand_pose.enable
            logger.debug(
                "Right hand pose weight, loss: {}, {}",
    ...
    def single_loss_step(
        ...
        if (
            self.left_hand_pose_weight > 0
            and self.loss_enabled["left_hand_pose"]
            and "left_hand_pose" in parameters
            and "left_hand_pose" in target_param_idxs
        ):
            ...
            if self.lhand_use_conf:
                ...
                weights = (
                    left_hand_conf.mean(axis=1, keepdim=True)
                    .expand(-1, num_left_hand_joints)
                    .reshape(-1)
                )
        ...
        if (
            self.right_hand_pose_weight > 0
            and self.loss_enabled["right_hand_pose"]
            and "right_hand_pose" in parameters
            and "right_hand_pose" in target_param_idxs
        ):
            ...
            if self.lhand_use_conf:
                weights = (
                    right_hand_conf.mean(axis=1, keepdim=True)
                    .expand(-1, num_right_hand_joints)
                    .reshape(-1)
                )
...
class SMPLXLossModule(SMPLHLossModule):
    ...
    def __init__(self, loss_cfg):
        ...
        self.expr_use_conf_weight = expression_cfg.get("use_conf_weight", False)
    ...
    def single_loss_step(
        ...
        compute_expr_loss = (
            self.expression_weight > 0
            and self.loss_enabled["expression"]
            and "expression" in parameters
            and "expression" in target_param_idxs
        )
        ...
        if (
            self.jaw_pose_weight > 0
            and self.loss_enabled["jaw_pose"]
            and "jaw_pose" in parameters
            and "jaw_pose" in target_param_idxs
        ):
            ...
            if self.jaw_use_conf_weight:
                ...
                weights = torch.ones(
                    [batch_size], dtype=est_jaw_pose.dtype, device=est_jaw_pose.device
                )
...
class RegularizerModule(nn.Module):
    def __init__(
        ...
        self.stages_to_regularize = loss_cfg.get("stages_to_penalize", [])
        ...
        if self.shape_prior_weight > 0:
            self.shape_prior = build_prior(shape_prior_type, **loss_cfg.shape.prior)
        ...
        if self.expression_prior_weight > 0:
            self.expression_prior = build_prior(
                expression_prior_type, **expression_prior_cfg
            )
        ...
        if self.body_pose_prior_weight > 0:
            self.body_pose_prior = build_prior(
                body_pose_prior_type, mean=body_pose_mean, **body_pose_prior_cfg
            )
        ...
        if self.right_hand_pose_prior_weight > 0:
            self.right_hand_pose_prior = build_prior(
                right_hand_pose_prior_type,
                mean=right_hand_pose_mean,
    ...
    def extra_repr(self) -> str:
        msg = []
        if self.shape_prior_weight > 0:
            msg.append("Shape prior weight: {}".format(self.shape_prior_weight))
        if self.expression_prior_weight > 0:
            msg.append(
                "Expression prior weight: {}".format(self.expression_prior_weight)
            )
        if self.body_pose_prior_weight > 0:
            msg.append("Body pose prior weight: {}".format(self.body_pose_prior_weight))
        if self.left_hand_pose_prior_weight > 0:
            msg.append(
                "Left hand pose prior weight: {}".format(
                    self.left_hand_pose_prior_weight
                )
            )
        if self.right_hand_pose_prior_weight > 0:
            msg.append(
                "Right hand pose prior weight {}".format(
                    self.right_hand_pose_prior_weight
                )
            )
        if self.jaw_pose_prior_weight > 0:
            msg.append("Jaw pose prior weight: {}".format(self.jaw_pose_prior_weight))
        return "\n".join(msg)
    ...
    def single_regularization_step(
        ...
        **kwargs,
        ...
        reg_shape = (
            self.shape_prior_weight > 0
            and betas is not None
            and not penalize_only_parts
        )
        if reg_shape:
            prior_losses["shape_prior"] = self.shape_prior_weight * self.shape_prior(
                betas, genders=genders
            )

        expression = parameters.get("expression", None)
        reg_expression = self.expression_prior_weight > 0 and expression is not None
        if reg_expression:
            prior_losses["expression_prior"] = (
                self.expression_prior(expression) * self.expression_prior_weight
            )

        body_pose = parameters.get("body_pose", None)
        betas = parameters.get("betas", None)
        reg_body_pose = (
            self.body_pose_prior_weight > 0
            and body_pose is not None
            and not penalize_only_parts
        )
        if reg_body_pose:
            prior_losses["body_pose_prior"] = (
                self.body_pose_prior(body_pose) * self.body_pose_prior_weight
            )

        left_hand_pose = parameters.get("left_hand_pose", None)
        if self.left_hand_pose_prior_weight > 0 and left_hand_pose is not None:
        ...
        if self.right_hand_pose_prior_weight > 0 and right_hand_pose is not None:
        ...
        if self.jaw_pose_prior_weight > 0 and jaw_pose is not None:
            prior_losses["jaw_pose_prior"] = (
                self.jaw_pose_prior(jaw_pose) * self.jaw_pose_prior_weight
            )

        return prior_losses

    def forward(
        self, network_params: Dict, targets: Optional[StructureList] = None, **kwargs
    ) -> Dict[str, Tensor]:
        ...
        genders = [
            t.get_field("gender") if t.has_field("gender") else "" for t in targets
        ]
        ...
        for ii, curr_key in enumerate(self.stages_to_regularize):
            ...
            curr_losses = self.single_regularization_step(curr_params, genders=genders)
            for key in curr_losses:
