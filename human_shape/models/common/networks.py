...
CONV_DIM_DICT = {1: nn.Conv1d, 2: nn.Conv2d, 3: nn.Conv3d}
...
def build_activation(activ_cfg) -> Union[nn.ReLU, nn.LeakyReLU, nn.PReLU]:
...
def build_norm_layer(input_dim: int, norm_cfg: Dict, dim: int = 1) -> nn.Module:
...
def build_rnn_cell(
    input_size: int, rnn_type="lstm", hidden_size=1024, bias=True
...
class FrozenBatchNorm2d(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return F.batch_norm(
            x, self.running_mean, self.running_var, self.weight, self.bias, False
        )
...
class MultiLayerRNNCell(nn.Module):
        ...
        **kwargs,
        ...
        if not learn_state:
            ...
            for n in range(num_states):
                self.hidden_state.append(
                    nn.Parameter(INIT_FUNCS[init_type](shape, dtype=torch.float32))
                )
        ...
        for ii, hidden_size in enumerate(layer_dims):
            self.rnn_list.append(
                rnn_cell_type(curr_input_dim, hidden_size=hidden_size, bias=bias)
            )
    ...
    def extra_repr(self):
        msg = [
            ...
            (f"Learned state init func: {self.init_type}" if self.learn_state else ""),
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for layer_idx, layer_dim in enumerate(layers):
            ...
            norm_layer = build_norm_layer(layer_dim, norm_cfg=normalization, dim=1)
...
class FCN(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        for layer_idx, layer_dim in enumerate(layers):
            activ = build_activation(activation)
            norm_layer = build_norm_layer(layer_dim, norm_cfg=normalization, dim=2)
            bias = norm_layer is None
...
class NonLocalBlock(nn.Module):
    def __init__(self, channels: int, dim=2, reduction: int = 2) -> None:
        ...
        scale = nn.Parameter(
            torch.zeros([1, channels] + [1] * dim, dtype=torch.float32)
        )
        self.register_parameter("scale", scale)

    def extra_repr(self) -> str:
        msg = [f"Dim: {self.dim}", f"Scale: {self.scale.shape}"]
        return "\n".join(msg)
...
def build_regressor(
    network_cfg, input_dim, output_dim, param_mean
    ...
    if regressor_type == "mlp":
        ...
        regressor = MLP(
            input_dim + append_params * param_mean.numel(), output_dim, **mlp_cfg
        )
        # Build the iterative regression object
        iterative_regressor = IterativeRegression(regressor, param_mean, **network_cfg)
        ...
        regressor = MultiLayerRNNCell(
            rnn_cfg,
            input_dim + append_params * param_mean.numel(),
            output_dim,
            **rnn_cfg,
        )
        # Build the iterative regression object
