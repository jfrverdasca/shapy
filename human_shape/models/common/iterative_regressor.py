...
class HMRLikeRegressor(nn.Module):
    def __init__(
        ...
        # Decide whether
        meas_definition_path = osp.expandvars(
            network_cfg.get("meas_definition_path", "")
        )
        meas_vertices_path = osp.expandvars(network_cfg.get("meas_vertices_path", ""))
        ...
        compute_measurements = (
            compute_measurements
            and osp.exists(meas_definition_path)
            and osp.exists(meas_vertices_path)
        )
        ...
        b2a_females_checkpoint_path = network_cfg.get("b2a_females_checkpoint", "")
        self.use_b2a = (
            use_b2a
            and osp.exists(b2a_males_checkpoint_path)
            and osp.exists(b2a_females_checkpoint_path)
        )
        if self.use_b2a:
            ...
            self.b2a_females = None  # B2A.load_from_checkpoint(
        ...
        a2b_females_checkpoint_path = network_cfg.get("a2b_females_checkpoint", "")
        self.use_a2b = (
            use_a2b
            and osp.exists(a2b_males_checkpoint_path)
            and osp.exists(a2b_females_checkpoint_path)
        )
        if self.use_a2b:
            logger.info(f"Loading A2B regressors ...")
            # load regressor for males
            self.a2b_males = None  # A2B.load_from_checkpoint(a2b_males_checkpoint_path)
            ...
            # load regressor for males
            self.a2b_females = (
                None  # A2B.load_from_checkpoint(a2b_females_checkpoint_path)
            )
    ...
    def _build_losses(self, loss_cfg):
        ...
        keypoint_loss_modules, center_around = self._build_keypoint_losses(loss_cfg)
        ...
        self.param_loss, self.regularizer = self._build_parameter_losses(loss_cfg)
        ...
        if self.mesh_edge_weight > 0:
            ...
            logger.debug(
                "3D Mesh Edges weight, loss: {}, {}",
                self.mesh_edge_weight,
                self.mesh_edge_loss,
            )
        ...
        if self.mesh_vertex_weight > 0:
            ...
            logger.debug(
                "3D Mesh vertices weight, loss: {}, {}",
                self.mesh_vertex_weight,
                self.mesh_vertex_loss,
            )
        ...
        if self.mass_weight > 0:
            ...
            logger.debug("Mass weight, loss: {}, {}", self.mass_weight, self.mass_loss)
        ...
        if self.height_weight > 0:
            ...
            logger.debug(
                "Height weight, loss: {}, {}", self.height_weight, self.height_loss
            )
        ...
        if self.chest_weight > 0:
            ...
            logger.debug(
                "Chest weight, loss: {}, {}", self.chest_weight, self.chest_loss
            )
        ...
        if self.waist_weight > 0:
            ...
            logger.debug(
                "Waist weight, loss: {}, {}", self.waist_weight, self.waist_loss
            )
        ...
        if self.hips_weight > 0:
            ...
            logger.debug("Hips weight, loss: {}, {}", self.hips_weight, self.hips_loss)
        ...
        if self.identity_weight > 0:
            ...
            self.loss_activ_step["identity"] = identity_loss_cfg.get("enable", 0)
            logger.debug(
        ...
        if self.attribute_weight > 0:
            ...
            self.loss_activ_step["attributes"] = attribute_loss_cfg.get("enable", 0)
            logger.debug(
        ...
        if self.beta_ref_weight > 0:
            ...
            # build_loss(**attribute_loss_cfg)
            self.loss_activ_step["beta_refined"] = beta_ref_loss_cfg.get("enable", 0)
            logger.debug(
                "Beta refined weight, loss {}, {}",
                self.beta_ref_weight,
                self.beta_ref_loss,
            )
        ...
        if self.mesh_vertex_ref_weight > 0:
            ...
            self.loss_activ_step["vertex_ref"] = vertex_ref_loss_cfg.get("enable", 0)
            logger.debug(
                "3D Mesh vertices weight, loss: {}, {}",
                self.mesh_vertex_ref_weight,
                self.mesh_vertex_ref_loss,
            )
    ...
    def compute_losses(
        ...
        **kwargs,
        ...
        gt_joints_3d_indices = torch.tensor(
            [ii for ii, t in enumerate(targets) if t.has_field("keypoints3d")],
            dtype=torch.long,
            device=device,
        )
        gt_joints3d = None
        ...
        est_vertices = out_params[f"stage_{num_stages - 1:02d}"]["vertices"]
        has_vertices = torch.tensor(
            [t.has_field("vertices") for t in targets], device=device, dtype=torch.bool
        )
        ...
        if has_vertices.sum().item() > 0:
            ...
            if self.mesh_edge_weight > 0:
                edge_loss_val = self.mesh_edge_loss(
                    gt_vertices=gt_vertices, est_vertices=est_vertices[has_vertices]
                )
                losses["mesh_edge_loss"] = self.mesh_edge_weight * edge_loss_val
            # Compute the vertex loss
            if self.mesh_vertex_weight > 0:
                vertex_loss_val = self.mesh_vertex_loss(
                    est_vertices[has_vertices], gt_vertices
                )
                losses["mesh_vertex_loss"] = self.mesh_vertex_weight * vertex_loss_val
        ...
        if self.compute_measurements:
            ...
            for meas_name, meas_data in out_params["measurements"].items():
                ...
                if len(gt_values) > 0 and loss_weight > 0:
                    ...
                    gt_values = torch.tensor(gt_values, device=device, dtype=dtype)
        ...
        if self.identity_weight > 0:
            ...
            for identity in unique:
                ...
                ii = torch.from_numpy(indices[np.where(mask)[0]]).to(device=device)

                # Compute all identity pairs
                pairs = torch.tensor(list(combinations(ii, 2)))

                # Compute the loss between the pairs
                losses["identity"] += self.identity_loss(betas[pairs[0]], pairs[[1]])
        ...
        if self.attribute_weight > 0:
            ...
            gt_attributes = torch.tensor(gt_attributes, device=device)
    ...
    def _build_pose_space(self, body_model_cfg) -> Dict[str, PoseParameterization]:
    ...
    def forward(
        ...
        **kwargs,
        ...
        regr_output = self.regressor(features, cond=cond, extra_features=extra_features)
        ...
        # the shape
        model_output = self.model(get_skin=True, return_shaped=True, **merged_params)
            ...
            if isinstance(model_output[key], (KeypointTensor,)):
                ...
                if len(out_list) == num_stages:
                    for ii, value in enumerate(out_list):
                        out_params[f"stage_{ii:02d}"][key] = KeypointTensor.from_obj(
                            value, curr_val
                        )
                else:
        ...
        if self.compute_measurements:
            ...
            measurements = self.body_measurements(shaped_triangles)["measurements"]
        ...
        if self.use_b2a:
            genders = [
                x.get_field("gender") if x.has_field("gender") else None
                for x in targets
            ]
            genders = np.array(
            ...
            attributes = torch.zeros(betas.shape[0], attributes_males.shape[1]).to(
                betas.device
            )
        ...
        if self.use_a2b:
            ...
            attr = torch.stack(
                [
                    (
                        torch.tensor(x.get_field("attributes"))
            ...
            male_height = torch.tensor(
                [t.get_field("height", 1.71) for t in targets],
                dtype=torch.float32,
                device=device,
            )
            female_height = torch.tensor(
                [t.get_field("height", 1.59) for t in targets],
                dtype=torch.float32,
                device=device,
            )

            male_weight = torch.tensor(
                [t.get_field("weight", 71.0) for t in targets],
                dtype=torch.float32,
                device=device,
            )
            female_weight = torch.tensor(
                [t.get_field("weight", 62.0) for t in targets],
                dtype=torch.float32,
                device=device,
            )
            ...
            betas_ref_males = self.a2b_males(male_feature_vec[genders_males, :])

            # logger.info(self.a2b_males)
            betas_ref_females = self.a2b_females(female_feature_vec[genders_females, :])
