...
class KeypointLosses(nn.Module):
    def __init__(
        ...
        if self.joints_2d_weight > 0:
            ...
            logger.debug(
                "2D joints weight, loss: {}", self.joints_2d_weight, self.joints_2d_loss
            )
        ...
        if self.joints_3d_weight > 0:
            ...
            logger.debug(
                "3D joints weight, loss: {}", self.joints_3d_weight, self.joints_3d_loss
            )
        ...
        if self.edge_2d_weight > 0:
            ...
            logger.debug(
                "2D edge weight, loss: {}", self.edge_2d_weight, self.edge_2d_loss
            )
        ...
        if self.edge_3d_weight > 0:
            ...
            logger.debug(
                "3D edge weight, loss: {}", self.edge_3d_weight, self.edge_3d_loss
            )
    ...
    def forward(
        ...
        if self.joints_2d_weight > 0:
            ...
            joints_2d_loss = self.joints_2d_weight * self.joints_2d_loss(
        ...
        if self.edge_2d_weight > 0 and self.edge_2d_active:
            edge_2d_loss = self.edge_2d_weight * self.edge_2d_loss(
                est_keypoints2d,
                gt_keypoints2d[:, :, :-1],
        ...
        if gt_keypoints3d is not None and len(gt_keypoints3d) > 0:
            ...
            if len(center_around) > 0:
                centering_indices = [
                    est_keypoints2d.keypoint_names.index(name)
                    for name in center_around
                    if name in est_keypoints2d._keypoint_names
                ]
                # Center the predictions using the estimated point
                est_center_point = est_keypoints3d[:, centering_indices, :].mean(
                    dim=1, keepdim=True
                )
                centered_pred_joints = est_keypoints3d - est_center_point

                gt_center_point = gt_keypoints3d[:, centering_indices, :-1].mean(
                    dim=1, keepdim=True
                )
                centered_gt_joints = gt_keypoints3d[:, :, :-1] - gt_center_point
            else:
            ...
            if self.joints_3d_weight > 0:
                joints_3d_loss = self.joints_3d_weight * self.joints_3d_loss(
            ...
            if self.edge_3d_weight > 0:
                edge_3d_loss = self.edge_3d_weight * self.edge_3d_loss(
                    est_keypoints3d[:, :],
                    gt_keypoints3d[:, :, :-1],
                    connections=connections,
