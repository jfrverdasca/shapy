...
class CameraParams(object):
    attributes = ["translation", "rotation", "scale", "focal_length", "scale_first"]

    KEYS = ["translation", "rotation", "scale", "focal_length", "scale_first"]
...
def build_cam_proj(camera_cfg, dtype=torch.float32):
    ...
    if camera_pos_scale == "softplus":
        ...
        raise ValueError(f"Unknown positive scaling function: {camera_pos_scale}")
    ...
    if camera_type.lower() == "persp":
        ...
        camera_mean = torch.tensor([mean_flength, 0.0, 0.0], dtype=torch.float32)
        camera_param_dim = 4
...
class PerspectiveCamera(nn.Module):
    ...
    def __init__(self, dtype=torch.float32, focal_length=None, **kwargs):
        ...
        # the camera matrix
        self.register_buffer("focal_length", torch.tensor(focal_length, dtype=dtype))
    ...
    def forward(
        ...
        **kwargs,
        ...
        if rotation is None:
            rotation = (
                torch.eye(3, dtype=points.dtype, device=device)
                .unsqueeze(dim=0)
                .expand(batch_size, -1, -1)
            )
        if translation is None:
            translation = (
                torch.zeros([3], dtype=points.dtype, device=device)
                .unsqueeze(dim=0)
                .expand(batch_size, -11)
            )

        if camera_center is None:
            camera_center = torch.zeros(
                [batch_size, 2], dtype=points.dtype, device=device
            )
        ...
        points_transf = torch.einsum(
            "bji,bmi->bmj", rotation, points
        ) + translation.unsqueeze(dim=1)

        img_points = torch.div(
            points_transf[:, :, :2], points_transf[:, :, 2].unsqueeze(dim=-1)
        )
        img_points = torch.einsum(
            "bmi,bji->bjm", camera_mat, img_points
        ) + camera_center.reshape(-1, 1, 2)
        return img_points
...
class WeakPerspectiveCamera(nn.Module):
    ...
    def forward(self, points, scale, translation, **kwargs):
        ...
        if self.scale_first:
            projected_points = (
                scale.view(-1, 1, 1) * points[:, :, :2]
            ) + translation.view(-1, 1, 2)
        else:
        ...
        if type(projected_points) != type(points) and isinstance(
            ...
            projected_points = KeypointTensor.from_obj(projected_points, points)
