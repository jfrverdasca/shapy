    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(RegressionResNet, self).__init__(
            block, layers, replace_stride_with_dilation=replace_stride_with_dilation
        )
    ...
    def extra_repr(self):
        if self.replace_stride_with_dilation is None:
            ...
            if not any(self.replace_stride_with_dilation):
                ...
                layer3 = layer2 * 2 ** (not self.replace_stride_with_dilation[1])
                layer4 = layer3 * 2 ** (not self.replace_stride_with_dilation[2])
                msg = [
                    f"Layer 1: {64 * self.expansion}, H / 4, W / 4",
...
if __name__ == "__main__":
    resnet = resnet50(pretrained=True, replace_stride_with_dilation=None)
