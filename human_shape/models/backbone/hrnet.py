...
class HighResolutionNet(nn.Module):
    ...
    def get_output_dim(self):
        base_output = {
            f"layer{idx + 1}": val for idx, val in enumerate(self.output_channels_dim)
        }
    ...
    def _make_conv_layer(
        ...
        for i in range(num_layers):
            ...
            layers.append(
                Bottleneck(in_channels, num_filters // 4, downsample=downsample)
            )
...
if __name__ == "__main__":
    from expose.config import parse_args

    exp_cfg = parse_args()

    hr_net_cfg = exp_cfg["network"]["hmr"]["backbone"]

    hrnet = build(hr_net_cfg)

    device = torch.device("cuda")
    hrnet = hrnet.to(device=device)
